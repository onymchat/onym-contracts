...
const VK_D5: &[u8] = &PLACEHOLDER_VK_BYTES;
const VK_D8: &[u8] = &PLACEHOLDER_VK_BYTES;
const VK_D11: &[u8] = &PLACEHOLDER_VK_BYTES;

const UPDATE_VK_D5: &[u8] = &PLACEHOLDER_VK_BYTES;
const UPDATE_VK_D8: &[u8] = &PLACEHOLDER_VK_BYTES;
const UPDATE_VK_D11: &[u8] = &PLACEHOLDER_VK_BYTES;

/// Synthesised 64-byte VK with sane fixed parameters. NOT a real
/// circuit. Replace with `include_bytes!` of a prover-emitted VK
/// once the PQ prover lands.
const PLACEHOLDER_VK_BYTES: [u8; VK_LEN] = build_placeholder_vk_bytes();

const fn build_placeholder_vk_bytes() -> [u8; VK_LEN] {
    ...
    // num_queries = 80
    b[8] = 80;
    // num_pi = VERIFIER_PI_COUNT_MEMBERSHIP = 16. The contract
    // routes to a different VK for update (VERIFIER_PI_COUNT_UPDATE
    // = 24); the placeholder uses 16 here as a stand-in. Real per-
    // circuit VKs will diverge in this field as well as in the
    // pinned root.
    b[12] = 16;
    // blowup_log = 1 (rate 1/2 = standard FRI)
    b[16] = 1;
    // pcs_pinned_root: 8 lanes, set to {1, 2, …, 8}. Real VKs
    // will carry the prover-baked Merkle root of the preprocessed
    // (constant) trace columns.
    let mut i = 0;
    while i < 8 {
        b[20 + i * 4] = (i as u8) + 1;
        i += 1;
    }
    // omega_0 = 3, omega_0_inv = 3^{-1} mod P (precomputed below),
    // two_inv = 2^{-1} mod P (precomputed below).
    //
    // P = 0x78000001 = 2_013_265_921.
    // 3^{-1} mod P = 1_342_177_281 = 0x50000001 (LE: 01 00 00 50)
    // 2^{-1} mod P = 1_006_632_961 = 0x3C000001 (LE: 01 00 00 3C)
    b[52] = 3;
    b[56] = 0x01;
    b[57] = 0x00;
    b[58] = 0x00;
